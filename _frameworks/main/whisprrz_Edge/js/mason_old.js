  ...
  return $.fn.mason = function(options, complete) {
    ...
    this.each(function() {
      ...
      setup = function() {
        ...
        if (settings.viewport && settings.numRows) {
            elements.block.height = $(settings.viewport)[0].offsetHeight/settings.numRows;// + 1
            settings.numCols=Math.round(parseFloat($(window).width()/elements.block.height));
        } else {
            elements.block.height = Math.round(parseFloat(($self.width() / columnSize()) / settings.ratio));//.toFixed(2);

        }
        elements.block.width = Math.round(parseFloat($self.width()/columnSize()+1));//.toFixed(2);+1
       // elements.block.width = Math.round($self.width()/columnSize()+1);
