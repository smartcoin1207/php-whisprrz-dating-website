...
class CEventsEventShow extends CHtmlBlock
{
    function parseBlock(&$html)
    {
        ...
        if($event)
        {
            ...
            $sql_event = "SELECT * FROM user WHERE user_id = " . to_sql($event['user_id']);
            $event_user = DB::row($sql_event);
            $name_seo = $event_user['name_seo'];
            $username = $event_user['name'];

            $photo_url = User::getPhotoDefault($event['user_id']);
