...
*/
	...
	Function InitializeOAuthSession(&$session)
	{
		...
		$session->creation = gmdate("Y-m-d H:i:s");
...
*/
	...
	Function ProcessToken1($oauth, &$access_token)
	{
		...
		elseif(IsSet($response['oauth_expires_in']))
		{
			...
			$this->access_token_expiry = gmdate('Y-m-d H:i:s', $this->response_time + $expires);
			if($this->debug)
	...
	Function RetrieveToken(&$valid)
	{
		...
		if(IsSet($access_token['value']))
		{
			...
			$this->access_token_expiry = '';
			$expired = (IsSet($access_token['expiry']) && strcmp($this->access_token_expiry = $access_token['expiry'], gmdate('Y-m-d H:i:s')) < 0);
			if($this->debug)
			{
				if($expired)
				{
	...
	Function CallAPI($url, $method, $parameters, $options, &$response)
	{
		...
		switch($version)
		{
			case 1:
				...
				&& strcmp($this->access_token_expiry, gmdate('Y-m-d H:i:s')) <= 0)
				{
	...
	Function CheckAccessToken(&$redirect_url)
	{
		...
		switch(intval($this->oauth_version))
		{
			case 1:
				...
				{
					$expired = (IsSet($access_token['expiry']) && strcmp($access_token['expiry'], gmdate('Y-m-d H:i:s')) <= 0);
			...
			case 2:
				...
				if(!$this->RetrieveToken($valid))
					return false;
				$expired = (strlen($this->access_token_expiry) && strcmp($this->access_token_expiry, gmdate('Y-m-d H:i:s')) <= 0 && strlen($this->refresh_token) === 0);
				if($valid
				&& !$expired)
					return true;
				if($this->debug)
