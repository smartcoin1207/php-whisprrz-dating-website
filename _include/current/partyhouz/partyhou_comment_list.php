...
class CpartyhouzpartyhouCommentList extends CHtmlBlock
{
	...
	function parseBlock(&$html)
	{
        ...
        if($partyhou)
        {
            ...
            $html->setvar('partyhou_title', strcut(to_html($partyhou['partyhou_title']), 24));
            $html->setvar('partyhou_title_full', to_html($partyhou['partyhou_title']));

        	$sql_base = CpartyhouzTools::comments_by_partyhou_sql_base($partyhou['partyhou_id']);

	        $n_results = CpartyhouzTools::count_from_sql_base($sql_base);

	        $page = intval(get_param('partyhou_comment_list_page', 1));
	        ...
	        $comments = CpartyhouzTools::retrieve_from_sql_base($sql_base, $n_results_per_page, ($page - 1) * $n_results_per_page);
	        ...
	        if(count($comments))
	        {
		        ...
		        foreach($comments as $comment)
		        {
		            ...
		            $html->setvar('comment_text', CpartyhouzTools::filter_text_to_html($comment['comment_text']));
	                ...
	                if($partyhou['user_id'] == $g_user['user_id'] || $comment['user_id'] == $g_user['user_id'])
	                ...
	                $comment_comments = CpartyhouzTools::retrieve_from_sql_base(CpartyhouzTools::comments_by_comment_sql_base($comment['comment_id']));
	                foreach($comment_comments as $comment_comment)
	                {
                        $html->setvar('comment_comment_id', $comment_comment['comment_id']);
                        $html->setvar('comment_comment_text', CpartyhouzTools::filter_text_to_html($comment_comment['comment_text'], true, $thumbnail_postfix, false));
	                    ...
	                    if($partyhou['user_id'] == $g_user['user_id'] || $comment_comment['user_id'] == $g_user['user_id'] || $comment['user_id'] == $g_user['user_id'])
