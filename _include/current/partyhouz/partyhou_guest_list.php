	...
	function parseBlock(&$html)
	{
        ...
        if($partyhou)
        {
            ...
            else
            {
                ...
                {
                    $html->parse('partyhou_guest_delete_title');
                }
                else
                {
            ...
            /* popcorn modified partyhou guest 2024-05-23 start */
            $guest_sql = "SELECT * FROM partyhouz_partyhou_guest WHERE partyhou_id = " . to_sql($partyhou['partyhou_id'], 'Text') . " AND user_id = " . to_sql(guid(), 'Text') . "";

            $guest_me = DB::row($guest_sql);
            if(isset($guest_me['user_id']) && $guest_me['user_id']) {
                if(Common::isOptionActive("partyhou_wall_enabled"))
                $html->parse('partyhou_wall_buttons');
            }
            /* popcorn modified partyhou guest 2024-05-23 start */
            ...
            {
				if($guest['user_id']==$partyhou['user_id']) $html->setvar('n_guests', l('Host'));
                ...
                if($partyhou['user_id'] != $guest['user_id'] && $partyhou['user_id'] == guid()) {
