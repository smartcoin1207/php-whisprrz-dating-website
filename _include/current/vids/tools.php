...
class CVidsTools
{
    ...
    static function addWherePrivateCondition($where = '')
    {
        ...
        if(!defined('ADMINISTRATOR')) {
            $where .= $and . '((private = "N") OR (private = "Y" AND user_id = ' . to_sql(guid(), 'Number') . ')) AND group_id = 0 '; // Divyesh - added on 17042024
        }
    ...
    static public function getNullVideo()
    {
        return self::_filterVideoToHtml(array(
            ...
            'tags'           => '',
            'active'         => '1',
            'private'        => 'N',    // Divyesh - added on 17042024
            'cat'            => '0',
        ));
    }
    ...
    static public function insertVideo($file="", $active=1, $isAddData = true, $isUploaded = 0, $notUploadFile = false)
    {
        ...
        DB::insert('vids_video', array(
            ...
            'private'        => (ses("video_upload_private") == '1' ? 'Y' : 'N'),   // Divyesh - added on 17042024
            'active'         => $active,
    ...
    {
        $private = par("private") == '1' ? 'Y' : 'N';   // Divyesh - added on 17042024
        $row = array(
        ...
        if(Common::getTmplSet() != 'old') {
            ...
            unset($row['tags']);
            unset($row['cat']);
            // unset($row['private']); // Divyesh - added on 17042024
...
class VideoUpload
{
    ...
    static function getPathToFile($file, $folder = '')
    {
        if(self::$converterClass === 'VideoUploadService') {
            ...
            if(getFileDirectoryType('video') == 2) {
                $pathToFile = Common::getOption('dir_files', 'path') . "temp/{$folder}/{$file}.txt";
            } else {
    ...
    static function upload($id, $file, $format = 'mp4', $folder = 'video', $outputImage = true)
    {
            ...
            if($id==0)
            {
                ...
                if(getFileDirectoryType('video') == 2) {
                    $file_id = $g['path']['dir_files'] . "temp/{$folder}/" . $file;
                } else {
                    $file_id = $g['path']['dir_files'] . "{$folder}/" . $file;
                }
                ...
                // var_dump($file, $folder, $file_id . '.' . $format, $imageFile, $format); die();

                // var_dump(self::getPathToFile($file, $folder), $file_id . '.' . $format, $imageFile, $format); die();
            ...
            if($id>0)
            {
                ...
                if(getFileDirectoryType('video') == 2) {
                    $file = $g['path']['dir_files'] . "temp/{$folder}/" . $file;
                    $file_new = $g['path']['dir_files'] . "temp/{$folder}/" . $id;
                } else {
                    $file = $g['path']['dir_files'] . "{$folder}/" . $file;
                    $file_new = $g['path']['dir_files'] . "{$folder}/" . $id;
                }
                ...
                if(getFileDirectoryType('video') == 2) {
                    ...
                    custom_file_upload($file_path, "{$folder}/" . $id . '.' . $format);
                    
                    //video image upload to s3
                    custom_file_upload($file_new . '.jpg', "{$folder}/" . $id . '.jpg');
                    custom_file_upload($file_new . '_src.jpg', "{$folder}/" . $id . '_src.jpg');
                    custom_file_upload($file_new . '_b.jpg', "{$folder}/" . $id . '_b.jpg');
                    custom_file_upload($file_new . '_bm.jpg', "{$folder}/" . $id . '_bm.jpg');
                }
    ...
    static function delete($id)
    {
            ...
            if(getFileDirectoryType('video') == 2) {
                ...
                custom_file_delete($file . '_bm.jpg');
                ...
                /* Live */
                if (custom_file_exists($file . '_bm.jpg')) unlink($file . '_bm.jpg');
                $fileTmp = $g['path']['dir_files'] . 'temp/live_image_' . $id . '_bm.jpg';
                if (custom_file_exists($fileTmp)) unlink($fileTmp);
                /* Live */
            }
    }
    static function urlFilesVideo($folder = 'video')
    {
        ...
        //popcorn modified s3 bucket video 2024-05-07
        if(getFileDirectoryType('video') == 2) {
            $url = Common::urlSiteSubfolders() . $g['dir_files'] . 'temp/' . $folder . '/';
        } else {
    ...
    static function downloadReadyFiles($id, $urlVideo, $fileSize)
    {
        ...
        if($id && $urlVideo) {
            ...
            //popcorn modified s3 bucket 2024-05-14
            if(getFileDirectoryType('video') == 2) {
                custom_file_upload($path[0], 'video/' . $id . '.mp4');
            }
        }
