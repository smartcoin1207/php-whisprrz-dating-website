var CWall = function() {
    ...
    /* ----------------------- Event, Hotdate, Partyhou Wall Codes Start----------------------------------- */
    this.getEHPWall = function() {
        isEHPWall = '';
        if(location.pathname.includes('event_wall.php')) {
            isEHPWall = "&is_ehp_wall=event";
        } else if(location.pathname.includes('hotdate_wall.php')) {
            isEHPWall = "&is_ehp_wall=hotdate";
        } else if(location.pathname.includes('partyhou_wall.php')) {
            isEHPWall = "&is_ehp_wall=partyhou";
        }

        return isEHPWall;
    }

    this.getEHPWallString = function() {
        isEHPWallString = '';

        if(location.pathname.includes('event_wall.php')) {
            isEHPWallString = "event";
        } else if(location.pathname.includes('hotdate_wall.php')) {
            isEHPWallString = "hotdate";
        } else if(location.pathname.includes('partyhou_wall.php')) {
            isEHPWallString = "partyhou";
        }

        return isEHPWallString;
    }


    /* ----------------------- Event, Hotdate, Partyhou Wall Codes End----------------------------------- */
    ...
    this.updater = function(isRefreshWall,call) {
        ...
        isEHPWall = $this.getEHPWall();

        var url='wall_ajax.php?cmd=update&last_item_id='+$this.getPostIdMax()+'&wall_uid='+$this.uid + isEHPWall,
...
//console.log($itemTemp.outerHeight(), $itemTemp.outerHeight(true),$itemTemp[0].offsetHeight, $itemTemp[0].clientHeight);
    ...
    this.itemsHistoryLoadItems = function() {
        ...
        var fnLoad=function(){
            $.ajax({url:'wall_ajax.php?cmd=items_old' + isEHPWall,
                type:'POST',
                data:data,
                timeout: globalTimeoutAjax,
                success: function(res){
    ...
    this.commentsLoadMore = function(id, $el) {
        ...
        var fnLoad=function(){
            $.ajax({url:'wall_ajax.php?cmd=comments_load' + isEHPWall,
                type:'POST',
                data:data,
                timeout: globalTimeoutAjax,
                success: function(res){
    ...
    this.likeAdd = function(id, $link,cmd) {
        ...
        $.post('wall_ajax.php?cmd='+cmd + isEHPWall, data, function(data){
            $icon.removeChildrenLoader();
    ...
    this.itemDelete = function(id) {
        ...
        isEHPWall = $this.getEHPWall();
        var url = 'wall_ajax.php?cmd=item_delete&id='+id+'&wall_uid='+$this.uid + isEHPWall;
    ...
    this.commentAdd = function(inp) {
        ...
        var fnAdd=function(){
            ...
            isEHPWall = $this.getEHPWall();
            url = 'wall_ajax.php?cmd=comment&id='+id+'&wall_uid='+$this.uid+'&last_id='+firstId + isEHPWall,
    ...
    this.commentAddReplies = function(inp) {
        ...
        var fnAdd=function(){
            ...
            isEHPWall = $this.getEHPWall();
                url = 'wall_ajax.php?cmd=comment&id='+id+'&wall_uid='+$this.uid+'&last_id='+lastId + isEHPWall,
                data = {comment:comment,
                        send:send,
                        section:section,
                        photo_id:param,
    ...
    this.commentDelete = function($el) {
        ...
        $.post('wall_ajax.php?cmd=comment_delete' + isEHPWall, data, function(res){
            var data=$this.checkDataAjax(res);
            if(data && $this.itemExists(id, data)){
                var $data=$(data);
                $this.$scriptUpdate.append($data.filter('script'));
    ...
    this.copyWallLink = function(link) {
      const input = document.createElement('input');
       full_link = window.location.origin  + '/' + link;
      input.value = full_link;
    ...
    this.makeShareable = function(id, $el) {

        isEHPWall = $this.getEHPWall();

        var url = 'wall_ajax.php?cmd=shareable&id=' + id + '&wall_uid=' + $this.uid + isEHPWall;
        var data = {};
        ...
        $.ajax({url:url,
                ...
                success: function(res){
                    ...
                    if(res_data.page == 'shareable') {
                        $el.find('span').text(shareable_title);

                    } else if(res_data.page == 'unshareable') {
                        $el.find('span').text(shareable_not_title);
                    } 
                },
                error: function(xhr, textStatus, errorThrown){
                    globalRetryAjaxTimeout(xhr, textStatus, errorThrown, function(){
    ...
    var confirmShareHtml = function(msg, handler, hCancel_or_title, title, btnOk, btnCancel, hideCancel, noCloseOk) {
        var fn=function(){
            ...
            var $pp=$('<div class="modal fade bs-example-modal-sm custom_modal custom_confirm_modal custom_modal_alert" tabindex="-1" role="dialog" data-backdrop="'+backdrop+'">'+
                    '<div class="modal-dialog modal-sm" role="document">'+
                        '<div class="modal-content">'+
                            titleHtml+
                            '<div class="modal-body share_list_modal"></div>'+
    ...
    this.getShareModalHtml = function(id, $el) {
        var msg={
            share : l('share_this_on_your_timeline'),
            unshare : l('unshare_on_your_timeline')
        }
        var cmd=$el.is('.share')?'share':'unshare';
        confirmShareHtml(msg[cmd],function(){$this.shareList(id, $el)},l('are_you_sure'));
        ...
        $.ajax({url:url,
                ...
                success: function(res){
                    ...
                    if($data) {
                        $('.modal-body.share_list_modal').html($data.html());
                    }
                },
                error: function(xhr, textStatus, errorThrown){
                    globalRetryAjaxTimeout(xhr, textStatus, errorThrown, function(){
    ...
    this.shareList = function(id, $el, cmd) {
        $el=$el||$($this.sel.feedShare+$this.guid+'_'+id);
        var $icon;
        if ($el[0]) {
            $el.addClass('disabled');
        ...
        var checkboxes = document.querySelectorAll('#wall_share_items input[type="checkbox"]');
        var checkboxValues = {};
        
        checkboxes.forEach((checkbox, index) => {
            var check_name = checkbox.name;
            var checkname = check_name.replace("wall_share_field_", "");
            checkboxValues[`${checkname}`] = checkbox.checked ? 1 : 0;
        });

        var commentValue = $("#share_comment_text1").val();

        var checked_shares = JSON.stringify({ checkboxValues });
        data = {
        ...
        cmd=cmd||($el.is('.share')?'share':'unshare');
        var url = 'wall_ajax.php?cmd=share_list&id=' + id + '&wall_uid=' + $this.uid + isEHPWall;
    ...
    this.likeCommentAdd = function($el, cid, rCid) {
        ...
        isEHPWall = $this.getEHPWall();
        $.post('wall_ajax.php?cmd=like_comment' + isEHPWall, data, function(res){
            var data=$this.checkDataAjax(res);
    ...
    this.changeAccess = function($el, id, access) {
        ...
        $.post('wall_ajax.php?cmd=update_access' + isEHPWall, {id:id,wall_uid:$this.uid,access:access}, function(data){
            data=$this.checkDataAjax(data);
            if(data && $this.itemExists(id, data)){
                $ac.removeClass('public private friends profile group');
                $ac.addClass(access);
                $this.updater();
            } else {
