...
class CPage extends CHtmlBlock
{
	function parseBlock(&$html)
	{
        ...
        if ($page == 1){
            ...
            if ($pid > 0) {
                $items = DB::all("SELECT * FROM gallery_images WHERE `albumid` = ". to_sql($pid,"Number")." LIMIT ". to_sql(($page - 1)*$pagerOnPage,"Number") . ",". to_sql($pagerOnPage,"Number") . ";");
            } else {
                $items = DB::all("SELECT * FROM gallery_images LIMIT ". to_sql(($page - 1)*$pagerOnPage,"Number") . ",". to_sql($pagerOnPage,"Number") . ";");
            }
        }
