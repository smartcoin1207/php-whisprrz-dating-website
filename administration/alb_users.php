...
class CPage extends CHtmlBlock
{
	function parseBlock(&$html)
	{
        ...
        $pagerOnPage = 20;
        $pagerUrl = g('path','url_administration') . 'alb_users.php?p=%s';

        $itemsTotal = DB::result('SELECT COUNT(DISTINCT `user_id`) FROM `gallery_albums`');

        $items = array();
        if ($page == 1){
            $ids = DB::all("SELECT DISTINCT `user_id` FROM gallery_albums LIMIT ". to_sql($pagerOnPage,"Number") . ";");
        }  else {
            $ids = DB::all("SELECT DISTINCT `user_id` FROM gallery_albums LIMIT ". to_sql(($page - 1)*$pagerOnPage,"Number") . ",". to_sql($pagerOnPage,"Number") . ";");
        }
