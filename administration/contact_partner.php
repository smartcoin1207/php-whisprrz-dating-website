...
class CForm extends CHtmlBlock
{
	...
	function action()
	{
		...
		if ($cmd == "delete")
		{
			...
			");
			$url = "contact_partner.php?action=delete";
		...
		if ($cmd == "delete_all"){
			$id = get_param_int('id');
			$mail = DB::result("SELECT mail FROM `contact_partner` WHERE `id` = " . to_sql($id));
			if ($mail) {
				DB::execute("DELETE FROM `contact_partner` WHERE `mail` = " . to_sql($mail));
				$url = "contact_partner.php?action=delete";
				if (get_param('order') == 'name') {
					$url .= '&order=name';
				}
		...
		if ($cmd == "answer")
		{
			...
			$url = "contact_partner.php?done=" . $id;
			if (get_param('order') == 'name') {
				$url .= '&order=name';
			}
			redirect($url);
	...
	function parseBlock(&$html)
	{
		...
		if ($isTmplModern) {
			...
			DB::query('SELECT * FROM contact_partner ORDER BY ' . $orderBy);
			$contacts = array();
			while ($row = DB::fetch_row()){
				...
				if (!isset($contacts[$key])) {
					...
					$contacts[$key]['photo'] = User::getPhotoDefault(0, 'r');
				...
				$contacts[$key]['messages'][] = array($row['comment'], $row['date'], $row['id'], $alias, $row['company'],  $row['phone']);
			}
			...
			foreach ($contacts as $key => $row) {
				...
				foreach ($messages as $k => $msg) {
					...
					$html->setvar('msg_name', $row['real_name']);
					$msgUserInfo = $msg[4];
					if ($msg[5]) {
						if ($msgUserInfo) {
							$msgUserInfo .= ', ';
					...
					$html->setvar('msg_user_info', $msgUserInfo);
					$html->subcond($msgUserInfo, 'msg_info');
