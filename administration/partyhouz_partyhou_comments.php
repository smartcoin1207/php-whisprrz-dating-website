...
class CPlaces extends CHtmlList
{
	...
	function init()
	{
		...
		$this->m_sql_count = "SELECT COUNT(m.partyhou_id) FROM partyhouz_partyhou_comment AS m " . $this->m_sql_from_add . "";
		$this->m_sql = "
			SELECT m.*
			FROM partyhouz_partyhou_comment AS m 
			" . $this->m_sql_from_add . "
		";
        ...
        if($partyhou_id)
        {
            $where .= " AND partyhou_id = " . to_sql($partyhou_id, 'Number');
        }
	...
	function onItem(&$html, $row, $i, $last)
	{
        ...
        $this->m_field['partyhou_id'][1] = DB::result("SELECT partyhou_title FROM partyhouz_partyhou WHERE partyhou_id=" . $row['partyhou_id'] . "", 0, 2);
        if ($this->m_field['partyhou_id'][1] == "") $this->m_field['partyhou_id'][1] = "blank";
...
$page = new CPlaces("main", $g['tmpl']['dir_tmpl_administration'] . "partyhouz_partyhou_comments.html");
