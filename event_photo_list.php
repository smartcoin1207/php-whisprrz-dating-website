...
class CPage extends CHtmlBlock
{
    ...
    function parseBlock(&$html)
    {
        ...
        $pageTitle = l('event_photo_list');

        $user_id = get_param('user_id', '');
        if($user_id) {
            $pageUrl = Common::pageUrl('event_photo', $event_id) . "&user_id=" . $user_id;
        } else {
            $pageUrl = Common::pageUrl('event_photo', $event_id);
        }
        ...
        $itemsTotal = CEventsTools::totalEventImages($event_id);
        ...
        if (!$ajax) {
            ...
            if ($guid) {
                    if (!$uid || $uid == $guid) {
                        ...
                        if (!$uid) {
                            $pageUrlMy = Common::pageUrl('event_photo', $event_id) . "&user_id=" . guid();
