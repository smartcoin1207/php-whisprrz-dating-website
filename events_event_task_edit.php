    ...
    function parseBlock(&$html)
    {
        ...
        if ($isEventSocial) {
            ...
            if($g_user['orientation'] == '1') {
                $orientation_sql = "u.set_male_post_my_calendar=1";
            } else if($g_user['orientation'] == '2') {
                $orientation_sql = "u.set_female_post_my_calendar=1";
            } else if($g_user['orientation'] == '5') {
                $orientation_sql = "u.set_couple_post_my_calendar=1";
            }

            $post_sql = "SELECT u.* FROM user as u LEFT JOIN friends_requests as fr ON fr.user_id = u.user_id " . 
            " WHERE u.user_id = " . to_sql($guid) . " OR ((fr.user_id = " . to_sql($guid) . " OR fr.friend_id = " . to_sql($guid) . ") AND u.set_friends_post_my_calendar = 1)".
            " OR " . $orientation_sql ." OR FIND_IN_SET(". to_sql($guid) .", u.set_post_my_calendar_users) > 0";

            $can_post_users = DB::rows($post_sql);
            if($can_post_users) {
                $blockFriend = 'list_friend_event';
                foreach ($can_post_users as $friend) {
                    $fid = $friend['user_id'];
