...
if($g_user['user_id'] != $hotdate['user_id']) {
    $hotdate_url = $g['path']['url_main'] . "hotdate_wall.php?hotdate_id" . $hotdate_id;
    redirect($hotdate_url);
}

class CHotdateMail extends CHtmlBlock
{
    ...
    public function action()
    {
        ...
        if ($cmd == "sent") {
            ...
            if ($saved_user_list_id == 'saved_all') {
                $user_sql = "SELECT user_id FROM hotdates_hotdate_guest WHERE hotdate_id = " . to_sql($hotdate_id, 'Number');
            } elseif($saved_user_list_id == 'saved_male') {
                $user_sql = "SELECT gs.user_id FROM hotdates_hotdate_guest AS gs LEFT JOIN user AS u ON gs.user_id = u.user_id WHERE gs.hotdate_id = " . to_sql($hotdate_id, 'Number') . " AND u.orientation=1";
            } elseif ($saved_user_list_id == 'saved_female') {
                $user_sql = "SELECT gs.user_id FROM hotdates_hotdate_guest AS gs LEFT JOIN user AS u ON gs.user_id = u.user_id WHERE gs.hotdate_id = " . to_sql($hotdate_id, 'Number') . " AND u.orientation=2";
            } elseif ($saved_user_list_id == 'saved_couple') {
                $user_sql = "SELECT gs.user_id FROM hotdates_hotdate_guest AS gs LEFT JOIN user AS u ON gs.user_id = u.user_id WHERE gs.hotdate_id = " . to_sql($hotdate_id, 'Number') . " AND u.orientation=5";
            } elseif ($saved_user_list_id == 'saved_transgender') {
                $user_sql = "SELECT gs.user_id FROM hotdates_hotdate_guest AS gs LEFT JOIN user AS u ON gs.user_id = u.user_id WHERE gs.hotdate_id = " . to_sql($hotdate_id, 'Number') . " AND u.orientation=6";
            }
            ...
            if ($selected_members && $subject != '' && $text != '') {
                ...
                if ($this->message_sent) {
                    ...
                    $url_to = $g['path']['url_main'] . "hotdate_wall.php?hotdate_id=" . $hotdate_id;
                    redirect($url_to);
                }
...
$page = new CHotdateMail("", $g['tmpl']['dir_tmpl_main'] . "event_mail_compose.html");
