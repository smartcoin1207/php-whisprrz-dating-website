    ...
    function action() {
        ...
        if ($cmd == "delete") {
            ...
            foreach ($id as $k => $v) {
                DB::execute("
					DELETE FROM friends
					WHERE user_id=" . $g_user['user_id'] . " AND fr_user_id=" . to_sql($v, "Number") . "
				");
            }
        }


        $favorite = DB::count('friends', '`user_id` = ' . to_sql(guid(), 'Number'));
        if ($favorite == 0) {
            // redirect('mail.php');
        }
...
$list->m_sql_from_add = "JOIN friends as i ON (u.user_id=i.fr_user_id AND i.user_id=" . $g_user['user_id'] . ")";
