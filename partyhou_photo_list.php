...
class CPage extends CHtmlBlock
{
    ...
    function parseBlock(&$html)
    {
        ...
        $pageTitle = l('partyhou_photo_list');

        $user_id = get_param('user_id', '');
        if($user_id) {
            $pageUrl = Common::pageUrl('partyhou_photo', $partyhou_id) . "&user_id=" . $user_id;
        } else {
            $pageUrl = Common::pageUrl('partyhou_photo', $partyhou_id);
        }
        ...
        $itemsTotal = CpartyhouzTools::totalPartyhouImages($partyhou_id);
        ...
        if (!$ajax) {
            ...
            if ($guid) {
                    if (!$uid || $uid == $guid) {
                        ...
                        if (!$uid) {
                            $pageUrlMy = Common::pageUrl('partyhou_photo', $partyhou_id) . "&user_id=" . guid();
