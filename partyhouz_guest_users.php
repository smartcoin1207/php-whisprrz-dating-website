...
class CPage extends CHtmlBlock
{
    ...
    function action() {
        ...
        if($data_cmd == 'guest_remove' && $ajax) {
            if($removeUid) {
                DB::execute("DELETE FROM partyhouz_partyhou_guest WHERE partyhou_id= " . to_sql($partyhou_id) . " AND user_id=" . to_sql($removeUid));
            }
        }
    }
    ...
    function parseBlock(&$html)
    {
        ...
        $sql_partyhou = "SELECT * FROM partyhouz_partyhou WHERE partyhou_id=" . to_sql($partyhou_id);
        $partyhou = DB::row($sql_partyhou);
        if(!$partyhou) {
            Common::toLoginPage();
        }

        $pageTitle = lSetVars('page_title', array('partyhou_title' => $partyhou['partyhou_title']));

        $vars = array('page_title'   => $pageTitle,
                      'page_description' => 'DDD',
                      'url_pages'    => './partyhouz_guest_users?partyhou_id='. $partyhou_id ,
                      'page_number' => $page,
                      'page_user_id' =>' $guid',
                      'page_param'   => 'page',
        ...
        $itemsTotal = DB::result('SELECT COUNT(*) FROM partyhouz_partyhou_guest F LEFT JOIN user as u ON u.user_id = F.user_id WHERE partyhou_id = ' . to_sql($partyhou_id) . $search_query_where);
        ...
        $rows = DB::rows(" SELECT u.*, DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(u.birth, '%Y') - (DATE_FORMAT(NOW(), '00-%m-%d') < DATE_FORMAT(u.birth, '00-%m-%d')) AS age FROM partyhouz_partyhou_guest as F LEFT JOIN user as u ON F.user_id=u.user_id WHERE F.partyhou_id = " . to_sql($partyhou_id) . $search_query_where .  " LIMIT " . $pagerOnPage . " OFFSET " . $offset);
