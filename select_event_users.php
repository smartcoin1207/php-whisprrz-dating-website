...
class CUsersResults extends CHtmlList
{
    public function action()
    {
        ...
        if (!self::isOwner()) {
            $redirect_url = $g['path']['url_main'] . "event_wall.php?event_id=" . $event_id;
            redirect($redirect_url);
        }

        $current_url = $g['path']['url_main'] . "select_event_users.php?event_id=" . $event_id;
        $cmd = get_param('cmd', '');
        $save = get_param('save', '');
        ...
        if ($save == 'all') {
            $members = CEventsTools::getGuestUsers($event_id);
            $selected_members = [];
        ...
        if($cmd == 'save' || $clear == 'all' || $save == 'all') {
            ...
            $exist_row = DB::row("SELECT * FROM " . $table . " WHERE event_id = " . to_sql($event_id) . " AND event_type = 'event'" );

            if(isset($exist_row)) {
                $sql = "UPDATE mass_mail_saved_user_list SET userlist = " . to_sql(json_encode($selected_members)) . " WHERE event_id = " . to_sql($event_id);
            } else {
                $sql = "INSERT INTO mass_mail_saved_user_list (event_id, userlist, event_type) values(" . to_sql($event_id, 'Text') . ", " . to_sql(json_encode($selected_members), 'Text') .  ", 'event')";
            }
    ...
    public function init()
    {
        ...
        $this->m_on_bar = 10;
        $this->m_sql_count = "SELECT COUNT(u.user_id) FROM events_event_guest AS gs " . $this->m_sql_from_add . "";

        $this->m_sql = "SELECT u.user_id, u.name FROM events_event_guest AS gs " . $this->m_sql_from_add . " ";

        $this->m_field['user_id'] = array("user_id", null);
        $this->m_field['name'] = array("name", null);

        $where = " AND event_id=" . to_sql($event_id, 'Text') . "AND u.user_id != " . to_sql($g_user['user_id'], 'Text');
    ...
    public function parseBlock(&$html)
    {
        ...
        $event_id = get_param('event_id', '');
        $gsql = "SELECT * FROM events_event_guest where event_id = " . to_sql($event_id, 'Text');
        $event_guests = DB::row($gsql);

        $url = $g['path']['url_main'] . "select_event_users.php?event_id=" . $event_id;
        $html->setvar('url_page', $url);
        $html->setvar('url_event_mail', $g['path']['url_main'] . "event_mail.php?event_id=" . $event_id);
