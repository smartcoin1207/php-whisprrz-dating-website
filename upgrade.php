...
function isUpgraded() {
    ...
    $result = false;
    if ($g_user['gold_days'] > 0 and $g_user['type'] != '' and $g_user['type'] != 'none' and $g_user['type'] != '0') {
...
class CGold extends CHtmlBlock {
    ...
    function parseBlock(&$html) {
        ...
        //popcorn delete start 2023-12-24
            // if ($optionTmplSet == 'urban') {
            //     $where = "`type` = 'membership'";
            //     $order = '`default` DESC,';
            // }
        //popcorn delete end 2023-12-24

        if ($optionTmplSet == 'urban') {
            $where = "`set` = 'urban' AND `type` = 'payment'";
            $order = '`default` DESC,';
        }
