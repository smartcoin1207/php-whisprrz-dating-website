...
class CViewTicket extends CHtmlBlock
{
    ...
    function action()
    {
        ...
        if ($cmd == "reply_ticket") {
            ...
            redirect("view_ticket.php?id={$id}");
        }
    }
    ...
    function parseBlock(&$html)
    {
        ...
        $id = get_param('id');
        $where = 'id = ' . to_sql($id) . ' AND user_from=' . to_sql($g_user['user_id']);
        $ticket = DB::one('support_tickets', $where);

        if (count($ticket) == 0) {
            redirect("support_tickets.php");
        } else {
            DB::update('ticket_replies', array('user_read' => '1'), '`ticket_id` = ' . to_sql($id, 'Number'));
...
$page = new CViewTicket("", $g['tmpl']['dir_tmpl_main'] . "view_ticket.html");
